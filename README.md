...
const SHARED_API_URL = "https://script.google.com/macros/s/AKfycbyn4BFbo-RaF0w1xUZ1upphJsisTdvVCj9ulRiD1Q_Axj6ruXxKaGcVFzmjgaiLKG9T/exec";
const LS_KEY = "hr_v15";
...
const SheetDB = function () {
  ...
  function applyState(d) {
    ...
    const arrClobber = (cur, next) => Array.isArray(next) && next.length === 0 && Array.isArray(cur) && cur.length > 0; // 빈값이 내용을 덮는 경우
    const objClobber = (cur, next) => next && typeof next === "object" && Object.keys(next).length === 0 && cur && typeof cur === "object" && Object.keys(cur).length > 0;
    ["team", "employees", "tasks", "routines", "memos"].forEach(k => {
      if (Array.isArray(d[k]) && !dirty.has(k)) {
        if (arrClobber(STORE[k], d[k])) return; // 비어있는 서버값으로 비어있지 않은 로컬을 덮어쓰지 않음(유실 방지)
        STORE[k] = d[k];
      }
    });
    ["ts", "gTmpl"].forEach(k => {
      if (d[k] && typeof d[k] === "object" && !Array.isArray(d[k]) && !dirty.has(k)) {
        if (objClobber(STORE[k], d[k])) return;
        STORE[k] = d[k];
      }
    });
    ...
    if (d.cl && typeof d.cl === "object") {
      ...
      Object.keys(d.cl).forEach(key => {
        if (!dirty.has("cl:" + key) && !objClobber(STORE.cl[key], d.cl[key])) STORE.cl[key] = d.cl[key];
      });
    }
    if (d.ins && typeof d.ins === "object") {
      ...
      Object.keys(d.ins).forEach(ym => {
        if (!dirty.has("ins:" + ym) && !objClobber(STORE.ins[ym], d.ins[ym])) STORE.ins[ym] = d.ins[ym];
      });
    }
  }
  function gasRun(fn) {
    ...
    // 폴백: google.script.run 이 없는 환경(Vercel/정적호스팅/로컬파일) → Apps Script /exec 로 fetch
    return gasFetch(fn, args);
  }
  async function gasFetch(fn, args) {
    if (fn === "getState") {
      const r = await fetch(SHARED_API_URL + "?action=getState&t=" + Date.now());
      if (!r.ok) throw new Error("HTTP " + r.status);
      return await r.json();
    }
    if (fn === "listDriveFiles") {
      const r = await fetch(SHARED_API_URL + "?action=listFiles&subFolderName=" + encodeURIComponent(args[0] || "default"));
      if (!r.ok) throw new Error("HTTP " + r.status);
      const d = await r.json();
      return (d && d.files) || (Array.isArray(d) ? d : []);
    }
    if (fn === "saveCollection") {
      await fetch(SHARED_API_URL, { method: "POST", mode: "no-cors", headers: { "Content-Type": "text/plain;charset=utf-8" }, body: JSON.stringify({ action: "replace", collection: args[0], value: args[1] }) });
      return true; // no-cors: 응답을 읽을 수 없어 낙관적 성공 처리(미러+이후 getState로 확인)
    }
    if (fn === "uploadDriveFile") {
      const b = args[0] || {};
      await fetch(SHARED_API_URL, { method: "POST", mode: "no-cors", headers: { "Content-Type": "text/plain;charset=utf-8" }, body: JSON.stringify({ action: "uploadFile", fileName: b.fileName, base64Data: b.base64Data, mimeType: b.mimeType, subFolderName: b.subFolderName }) });
      return { ok: true };
    }
    throw new Error("알 수 없는 작업: " + fn);
  }
  async function boot() {
    emit("loading");
    const mir = readMirror();
    if (mir) applyState(mir);
    try {
  ...
  async function diagnose() {
    const hasGAS = typeof google !== "undefined" && google.script && google.script.run;
    const mode = hasGAS ? "google.script.run (Apps Script 호스팅)" : "fetch (외부 호스팅/로컬)";
    try {
      await gasRun("getState");
    } catch (e) {
      return {
        ok: false,
        stage: "read",
        msg: "[전송 방식: " + mode + "]\n서버 읽기에 실패했습니다 (" + (e && e.message || e) + ").\n외부 호스팅(Vercel/로컬)이라면 저장용 Apps Script 배포의 '액세스 권한'이 반드시 '모든 사용자'여야 합니다."
      };
    }
    const token = "diag_" + Date.now();
    try {
      await gasRun("saveCollection", "__diag__", token);
    } catch (e) {
      return {
        ok: false,
        stage: "write",
        msg: "[전송 방식: " + mode + "]\n쓰기(저장)에 실패했습니다 (" + (e && e.message || e) + ")."
      };
    }
    let d2;
    try {
      d2 = await gasRun("getState");
    } catch (e) {
      return {
        ok: false,
        stage: "verify",
        msg: "확인용 읽기 실패: " + (e && e.message || e)
      };
    }
    try {
      await gasRun("saveCollection", "__diag__", null);
    } catch (_) {}
    if (d2 && d2.__diag__ === token) return {
      ok: true,
      stage: "ok",
      msg: "✓ 정상입니다. 읽기와 쓰기(저장) 모두 서버에 반영됩니다.\n[전송 방식: " + mode + "]"
    };
    return {
      ok: false,
      stage: "write",
      msg: "저장은 전송됐지만 다시 읽었을 때 값이 확인되지 않았습니다.\nApps Script 코드가 google.script.run 용 함수(getState/saveCollection)를 포함한 최신본인지 확인하세요."
    };
  }
  return {
    boot,
...
function SummaryBar({
  ...
  return React.createElement("div", {
    ...
    className: "stat-card",
    style: {
      borderTopColor: thisWeek.length > 0 ? "#EF4444" : "var(--sky)"
    }
  }, React.createElement("div", {
    className: "stat-label",
    style: {
      color: thisWeek.length > 0 ? "#DC2626" : "var(--t3)"
    }
  }, "\uAE34\uAE09 \uB9C8\uAC10"), React.createElement("div", {
    style: {
      display: "flex",
      alignItems: "center",
      gap: 12
    ...
    className: "stat-value",
    style: {
      color: thisWeek.length > 0 ? "#EF4444" : "var(--t3)",
      fontSize: 30
    }
  }, thisWeek.length), React.createElement("div", null, React.createElement("div", {
    className: "stat-sub"
  }, "\uAC74 \uC774\uBC88 \uC8FC \uB9C8\uAC10"), thisWeek.slice(0, 2).map(t => React.createElement("div", {
    key: t.id,
    style: {
      fontSize: 10,
      color: "#DC2626",
  ...
  }, dday(t.due) + " · " + t.task))))), React.createElement("div", {
    className: "stat-card",
    style: {
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async e => {
      try {
        ...
        await SheetDB.gasRun("uploadDriveFile", {
          fileName: taggedName,
          base64Data: base64,
...
function driveFileKind(name) {
  const n = String(name || "");
  if (n.indexOf("[건강보험]") >= 0 || n.indexOf("건강") >= 0) return "건강보험";
  if (n.indexOf("[국민연금]") >= 0 || n.indexOf("연금") >= 0) return "국민연금";
  if (n.indexOf("[급여대장]") >= 0 || n.indexOf("급여") >= 0) return "급여대장";
  return "기타";
}
async function driveFetchFileList(curYM) {
  try {
    const files = await SheetDB.gasRun("listDriveFiles", curYM);
    return Array.isArray(files) ? files : files && files.files || [];
  } catch (e) {
    return [];
  }
}
...
function Page4Insurance({
  ...
  function uploadToDrive(file, kind) {
    if (!(GAS_DRIVE_URL && GAS_DRIVE_URL !== "여기에_GAS_배포_URL_입력")) return;
  ...
  return React.createElement("div", {
    ...
    strokeWidth: "1.4",
    strokeLinecap: "round"
  })), showDrivePanel ? "드라이브 패널 닫기" : "구글 드라이브 원본 파일 보기"), driveUploading && React.createElement("span", {
    style: {
      fontSize: 10,
      color: "var(--sky-d)",
      fontWeight: 600
    ...
    strokeWidth: "1.6",
    strokeLinejoin: "round"
  })), "구글 드라이브 — " + curYM + " 원본 파일"), React.createElement("button", {
    className: "btn btn-ghost btn-sm",
    style: {
      fontSize: 10
    },
...
function Dashboard() {
  ...
  const reconcile = useCallback(data => {
    ...
    const setIfDiff = (cur, next, setter) => {
      ...
      if (emptyNext && nonEmptyCur) return; // 빈 서버값으로 내용 있는 로컬을 덮어쓰지 않음(유실 방지)
      setter(next);
    };
  ...
  return React.createElement("div", {
    ...
    className: "ldot"
  }), "\uAD6C\uAE00 \uC2DC\uD2B8 \uC2E4\uC2DC\uAC04 \uACF5\uC720 \uC800\uC7A5"), React.createElement("div", {
    style: {
      fontSize: 9,
  ...
  }, "+ \uC9C1\uC6D0 \uCD94\uAC00"), React.createElement("button", {
    className: "btn btn-ghost",
    style: {
      fontSize: 11,
      padding: "5px 12px"
...
function App() {
  ...
  if (!booted) {
    ...
    }), "\uAD6C\uAE00 \uC2DC\uD2B8\uC5D0\uC11C \uACF5\uC720 \uB370\uC774\uD130\uB97C \uBD88\uB7EC\uC624\uB294 \uC911\u2026"), React.createElement("style", null, "@keyframes bootspin{to{transform:rotate(360deg)}}.boot-spin{animation:bootspin .7s linear infinite}"));
  }
